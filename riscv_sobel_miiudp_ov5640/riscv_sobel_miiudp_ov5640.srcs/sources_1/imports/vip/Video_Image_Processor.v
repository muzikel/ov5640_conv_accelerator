module Video_Image_Processor
    #(
    parameter  SOBEL_THRESHOLD = 128//sobel阈�??
    )
    (
    input   clk,    //cmos 像素时钟
    input   rst_n,  
    ...
    //处理后的图像数据
    output       out_frame_vsync, //处理后的图像数据列有效信�?  
    output       out_frame_href,  //处理后的图像数据行有效信�?  
    output       out_frame_clken, //处理后的图像数据输出使能效信�?
...
VIP_Sobel_Edge_Detector 
    #(
    .SOBEL_THRESHOLD  (SOBEL_THRESHOLD)//sobel阈�??
    )
u_VIP_Sobel_Edge_Detector(
    ...
    .per_frame_clken (post_frame_de), //预处理图像使能信�?
    ...
    //用户接口
//    .Sobel_Threshold  (Sobel_Threshold) //Sobel 阈�??
);
